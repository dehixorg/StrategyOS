...
const { generateStrategy, analyzeStrategy, chatWithStrategy } = require('../services/openai')
...
// POST /ai/chat/:strategyId
router.post('/chat/:strategyId', auth, async (req, res) => {
  try {
    ...
    const reply = await chatWithStrategy(strategy, executions, message)
